...
| Hybrid Multi-View + Random Ratio | 已实现、已实验 | 固定预算对照显示 semantic-view 价值有正有负 | 保留为增强消融 |
| Random-Ratio 7 | 已实现、已实验 | 当前多数 CVR 和 merged 指标最强的增强方案 | 优先增强候选 |
...
完成固定预算对照后，需要修正此前对 Hybrid random4 的初步解释。

`Hybrid random1` 通过用最多 3 个 semantic views 替代 3 个 random views，与原始 4x 的最大视图预算近似一致。相对原始 4x，它使 CVR `HR@5` 提升 `2.23%`、`HR@10` 提升 `1.98%`、`R@5` 提升 `2.90%`、`R@10` 提升 `1.81%`；但 `HR@1/R@1` 下降，`N@5/N@10` 基本不变，merged behavior 也基本不变。因此 semantic views 确实表现出一定的 CVR coverage 导向，但没有形成固定预算下的全面排序增益。

`Random7` 使用纯 random-ratio views，与 Hybrid random4 的最大视图预算近似一致。相对 Hybrid random4，Random7 的 CVR `HR@10` 提升 `1.53%`、`R@5` 提升 `2.64%`、`R@10` 提升 `2.78%`、`N@5` 提升 `0.82%`、`N@10` 提升 `1.19%`；四个主要 merged 指标 `HR@5/HR@10/N@5/N@10` 也均提升。相对原始 4x，Random7 的 CVR `HR@5/HR@10/N@5/N@10` 分别提升 `6.17%/4.13%/3.71%/2.95%`，merged `HR@5/HR@10/N@5/N@10` 分别提升 `2.95%/2.17%/2.70%/2.64%`。其主要代价是 CVR `HR@1/R@1` 下降，且 merged rank-1 指标相对 Hybrid random4 略低。

因此，固定预算证据不支持将 semantic hybrid augmentation 作为默认主方法。Hybrid random4 的大部分收益可以由更大的视图预算解释；在当前 recent/hierarchy/session semantic policies 下，增加 random-ratio views 比替换成 semantic views 更有效。Hybrid random1 仍可作为说明目标行为 coverage 重分配的消融，而 Random7 是当前优先的实用增强候选。最终论文配置确定前仍需进行多随机种子验证和跨 backbone 迁移实验。Hybrid 方案继续通过 `sequence_augmentation=multi_view` 下的 `--multi_view_random_ratio_views N` 启用。Curriculum 会越过当前静态 cache 边界，需要 Dataset、sampler、Trainer、DDP 和 resume 行为协同修改，因此继续暂缓。
