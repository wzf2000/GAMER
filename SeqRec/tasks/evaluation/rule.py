...
class _BaseRuleTask(Task):
    """Shared skeleton for rule-based predictor tests.

    Subclasses implement:
      - _load_test_dataset(...)
      - check_collision_items()
      - test_all_behaviors(num_items, unique)
    """

    @staticmethod
    def _add_common_rule_args(parser, default_test_task: str):
        ...
        parser.add_argument("--test_task", type=str, default=default_test_task)
        return parser

    def _load_test_dataset(self, *, dataset: str, data_path: str, max_his_len: int, index_file: str, test_task: str):
        raise NotImplementedError

    def check_collision_items(self) -> list[dict[str, int | float]]:
        raise NotImplementedError

    def test_all_behaviors(self, num_items: int, unique: bool) -> dict[str, dict[str, float]]:
        raise NotImplementedError

    def _merge_behavior_results(self, behavior_results: dict[str, dict[str, float]]) -> list[dict[str, float]]:
        results: list[dict] = []
    ...
    def invoke(
        ...
        **kwargs,
        ...
        self.dataset = self._load_test_dataset(
            dataset=dataset,
            data_path=data_path,
        ...
        behavior_results = self.test_all_behaviors(num_items, unique)
        results = self._merge_behavior_results(behavior_results)
...
class TestMBRule(_BaseRuleTask):
    """
    ...
    def add_sub_parsers(sub_parsers: SubParsersAction):
        ...
        _BaseRuleTask._add_common_rule_args(parser, default_test_task="mb_explicit")

    def _load_test_dataset(self, *, dataset: str, data_path: str, max_his_len: int, index_file: str, test_task: str) -> BaseMBDataset:
        return load_MB_test_dataset(dataset, data_path, max_his_len, index_file, test_task)
...
class TestSMBRule(_BaseRuleTask):
    """
    ...
    def add_sub_parsers(sub_parsers: SubParsersAction):
        ...
        _BaseRuleTask._add_common_rule_args(parser, default_test_task="SeqRec")

    def _load_test_dataset(self, *, dataset: str, data_path: str, max_his_len: int, index_file: str, test_task: str) -> BaseSMBDataset:
        return load_SMB_test_dataset(dataset, data_path, max_his_len, index_file, test_task)
        ...
        for sample in dataset:
            ...
            for behavior in behaviors_set:
                ...
                if unique:
                    ...
                    history = sample["inters_item_list"][::-1][:num_items]
                output_str = [dataset.get_behavior_item(item, behavior) for item in history]
