...
class Qwen3SessionMultiDecoderLayer(nn.Module):
    def __init__(self, config: Qwen3MoeConfig, layer_idx: int, is_sparse: bool, behavior_injection: bool, is_cross: bool):
        ...
        self.self_attn = Qwen3SessionMultiAttention(config=config, layer_idx=layer_idx, is_cross=False)

        if self.is_cross:
            self.cross_attn = Qwen3SessionMultiAttention(config=config, layer_idx=layer_idx, is_cross=True)
...
class Qwen3SessionMultiModelBase(Qwen3DecoderModelBase):
    """
    ...
    decoder_layer_cls = Qwen3SessionMultiDecoderLayer
...
class Qwen3SessionMultiWithTemperature(ExtendedSessionPositionMixin, Qwen3ForCausalLM):
    def __init__(self, config: Qwen3MoeConfig):
        super(Qwen3ForCausalLM, self).__init__(config)
        self.init_custom_causal_lm(config, Qwen3SessionMultiModel)
