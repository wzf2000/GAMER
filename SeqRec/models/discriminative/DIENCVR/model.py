...
class DIENCVR(SeqModel):
    def __init__(self, config: DIENCVRConfig, n_items: int, max_his_len: int, n_behaviors: int, **kwargs):
        super(DIENCVR, self).__init__(config, n_items)
    ...
    def _define_parameters(self):
        ...
        self.candidate_projection = nn.Linear(self.embedding_size, self.hidden_size)
        self.attention = nn.Sequential(
            nn.Linear(self.hidden_size * 4, self.attention_hidden_size),
            nn.PReLU(),
            nn.Linear(self.attention_hidden_size, 1),
        )

        layers = []
        in_size = self.hidden_size * 3
    ...
    def _define_loss(self, loss_type: str):
        if loss_type != "BCE":
            raise NotImplementedError("DIENCVR only supports BCE loss.")
    ...
    def forward(self, item_seq: torch.Tensor, behavior_seq: torch.Tensor, candidate_item: torch.Tensor) -> torch.Tensor:
        ...
        evolution_states, _ = self.gru(history_emb)
        candidate_state = self.candidate_projection(self.item_embedding(candidate_item))
        candidate_expanded = candidate_state[:, None, :].expand_as(evolution_states)

        attention_input = torch.cat(
            [
                evolution_states,
                candidate_expanded,
                evolution_states * candidate_expanded,
                evolution_states - candidate_expanded,
            ],
            dim=-1,
        )
        ...
        user_interest = torch.sum(evolution_states * attention_weight[:, :, None], dim=1)

        logits = self.mlp(torch.cat([user_interest, candidate_state, user_interest * candidate_state], dim=-1))
