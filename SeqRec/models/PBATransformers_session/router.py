...
class PBAEncoderRouterSession(nn.Module):
    """
    ...
    def __init__(self, num_items: int, config: PBATransformerConfigSession):
        super().__init__()
        ...
        self.use_session_token = config.use_session_token
    ...
    def forward(
        self, input_id_sequence: torch.Tensor, cache_position: torch.Tensor | None = None, session_ids: torch.Tensor | None = None, times: torch.Tensor | None = None
        ...
        if self.use_session_token:
            session_indices = session_ids[:, :seq_length - 1]
            session_indices = torch.cat(
                (
                    session_indices,  # item semantic tokens
                    torch.zeros(
                        batch_size,
                        1,
                        dtype=torch.long,
                ...
                dim=1,)
            session_indices[(input_id_sequence == self.pad) | (input_id_sequence == self.eos)] = -2
        else:
            session_indices = torch.zeros(
                batch_size,
                seq_length,
                dtype=torch.long,
                device=input_id_sequence.device,
            ) - 2
        time_ids = times[:, :seq_length - 1]
        time_ids = torch.cat(
            (
                time_ids,
                torch.zeros(
            ...
            dim=1,)
        time_ids[(input_id_sequence == self.pad) | (input_id_sequence == self.eos)] = -1
        return position_index, behavior_indices, session_indices, time_ids


class PBADecoderRouterSession(nn.Module):
    def __init__(self, num_items: int, config: PBATransformerConfigSession):
        super().__init__()
        self.num_items = num_items
        self.num_experts = config.num_experts
        ...
        self.use_behavior_token = config.use_behavior_token
        self.use_session_token = config.use_session_token
        pre_generated_position_index = [torch.zeros(1, dtype=torch.long)]  # BOS token
        if (
            config.Moe_behavior_only
        ):  # If we only use one expert for the item semantic tokens and another for other tokens
            if self.use_behavior_token:
                ...
                pre_generated_position_index.append(
                    (
                        torch.tensor(
                            [1] * self.num_positions, dtype=torch.long
                        )
                    ).repeat(self.num_items)
                )
        else:
        ...
        if self.use_behavior_token:
            self.behavior_token_indices = torch.arange(
                0, self.num_items * self.num_positions, self.num_positions
            ) + 1  # The indices of the behavior tokens in the input sequence
        else:
            self.behavior_token_indices = (
                None  # If we don't use behavior token, we don't need the indices
            )
    ...
    def forward(
        ...
        session_indices = torch.zeros(batch_size, seq_length, dtype=torch.long, device=input_id_sequence.device) - 2
        if self.use_session_token:
            session_indices[position_index > 0 & (position_index < self.num_positions + 1)] = -1
        times_ids = torch.zeros(batch_size, seq_length, dtype=torch.float, device=input_id_sequence.device) - 1
        times_ids[position_index > 0 & (position_index < self.num_positions + 1)] = 0
        return position_index, behavior_indices, session_indices, times_ids
