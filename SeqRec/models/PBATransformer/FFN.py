...
class PBATransformersDenseActDenseHalfOutput(T5DenseActDense):
    def __init__(self, config: PBATransformerConfig, behavior_injection: bool = False):
...
class PBATransformersSparseMLP(nn.Module):
    ...
    def __init__(
        ...
        expert_class: nn.Module = PBATransformersDenseActDense,
        ...
        if config.shared_expert and is_sparse:
            ...
            self.shared_expert = PBATransformersDenseActDenseHalfOutput(
...
class PBATransformersLayerFF(nn.Module):
    ...
    def __init__(
        self, config: PBATransformerConfig, is_sparse: bool = False, behavior_injection: bool = False
    ):
        super().__init__()
        self.is_sparse = is_sparse

        # Check if it is a sparse layer, if not then it is a dense layer
        self.mlp = PBATransformersSparseMLP(
    ...
    def forward(self, hidden_states: torch.Tensor, position_index: torch.Tensor, behavior_index: torch.Tensor) -> torch.Tensor:
