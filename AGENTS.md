...
- Script environment variables use `name=value` prefixes before `bash`; extra task arguments should be appended after the script path with normal argparse syntax, for example:
  ...
  - `dataset=ShortVideoAD gpu=0 bash scripts/train_SMB_decoder.sh --num_train_epochs 20 --learning_rate 3e-4 --warmup_ratio 0.04 --debug`
- Prefer passing training hyperparameters after the script path with argparse/Hugging Face-style names instead of adding new script environment variables.
